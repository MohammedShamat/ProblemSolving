...
void MonthCalender(short year, short month) {
	short dayInMonth = DaysInMonth(year, month);//numberofdays
	short dayNumber = DayNumberInWeek(year, month, 1);//curr
	cout << "  ________________" << NameOfMonth(month) << "______________\n\n";
	cout << "  ";
...
int main() {
	...
	MonthCalender(yearFromUser, monthFromUser);
